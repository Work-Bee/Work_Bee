...
# Kill any existing processes on ports 3333 and 5555
...
echo "🔧 Starting Backend API Server (Port 5555)..."
...
echo "🎨 Starting Frontend Server (Port 3333)..."
cd /home/kiran/mern_jobportal/frontend
nohup serve -s build -l 3333 > ../logs/frontend.log 2>&1 &
FRONTEND_PID=$!
...
if lsof -Pi :5555 -sTCP:LISTEN -t >/dev/null ; then
    echo "✅ Backend Server: RUNNING on http://localhost:5555"
else
    echo "❌ Backend Server: FAILED TO START"
fi

if lsof -Pi :3333 -sTCP:LISTEN -t >/dev/null ; then
    echo "✅ Frontend Server: RUNNING on http://localhost:3333"
else
...
echo "🔗 Backend API available at: http://localhost:5555"
echo ""
